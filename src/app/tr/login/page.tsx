...
function LoginContent() {
  ...
  useEffect(() => {
    ...
    if (spotifyToken) {
      ...
      router.replace('/tr/login?spotify=connected'); // Update to use locale path
    }
  ...
  const redirectUri = 'https://spotify-to-youtube-pink.vercel.app/tr/login';
  ...
  const handleSpotifyCode = async (code: string) => {
    try {
      ...
      if (data.access_token) {
        ...
        router.replace('/tr/login?spotify=connected');
      } else {
  ...
  const handleYoutubeSuccess = (credentialResponse: CredentialResponse) => {
    ...
    if (credentialResponse.credential) {
      ...
      const redirectUri = encodeURIComponent(window.location.origin + '/tr/login');
        ...
        <div className="mt-8 text-center">
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            {isYoutubeConnected && isSpotifyConnected ? (
              <Link href="/tr/playlists" className="inline-flex items-center justify-center py-3 px-6 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-lg transition-colors text-base shadow-sm hover:shadow-md">
                <span>Çalma Listelerinize Gidin</span>
              ...
              <div className="flex flex-row items-center justify-center text-sm text-yellow-700 dark:text-yellow-500 bg-yellow-50 dark:bg-yellow-900/10 p-3 rounded-md border border-yellow-100 dark:border-yellow-800">
                ...
                <span>Çalma listelerinize erişmek için lütfen her iki hesaba da bağlanın</span>
              </div>
            )}
          </div>
          
          <Link href="/tr" className="inline-block mt-6 text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300">
            <span className="flex items-center">
