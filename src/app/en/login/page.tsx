...
function LoginContent() {
  ...
  useEffect(() => {
    ...
    if (spotifyToken) {
      ...
      router.replace('/en/login?spotify=connected'); // Update to use locale path
    }
  ...
  const redirectUri = 'https://spotify-to-youtube-pink.vercel.app/en/login';
  ...
  const handleSpotifyCode = async (code: string) => {
    try {
      ...
      if (data.access_token) {
        ...
        router.replace('/en/login?spotify=connected');
      } else {
        console.error('Invalid code or error occurred:', data.error || 'Unknown error');
      }
    } catch (error) {
  ...
  const handleSpotifyConnect = () => {
    ...
    alert('You will be redirected to the Spotify authorization page. After authorization, you will automatically return to the application.');
  ...
  const handleYoutubeSuccess = (credentialResponse: CredentialResponse) => {
    ...
    if (credentialResponse.credential) {
      ...
      const redirectUri = encodeURIComponent(window.location.origin + '/en/login');
          ...
          <div className={`p-4 sm:p-5 border border-gray-200 dark:border-gray-700 rounded-lg relative 
            ...
            {!isSpotifyConnected && (
              <div className="mt-4 bg-blue-50 dark:bg-blue-900/10 p-3 rounded-md border border-blue-100 dark:border-blue-800">
                <h3 className="text-sm font-medium text-blue-800 dark:text-blue-300 mb-2">Connection Instructions:</h3>
                <ol className="text-xs text-blue-700 dark:text-blue-400 space-y-1 list-decimal pl-4">
                  <li>You will be redirected to the Spotify authorization page</li>
                  <li>Log in to your Spotify account and grant permission</li>
                  <li>After authorization, you will automatically return to this page</li>
                </ol>
              </div>
            )}
          </div>
          ...
          <div className={`p-4 sm:p-5 border border-gray-200 dark:border-gray-700 rounded-lg relative
            ...
            <div className="mb-4">
              <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                Use the button below to connect to your YouTube account. Access permission is required to transfer playlists to YouTube Music.
              </p>
        ...
        <div className="mt-8 text-center">
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            {isYoutubeConnected && isSpotifyConnected ? (
              <Link href="/en/playlists" className="inline-flex items-center justify-center py-3 px-6 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-lg transition-colors text-base shadow-sm hover:shadow-md">
                <span>Go to Your Playlists</span>
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 ml-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7l5 5m0 0l-5 5m5-5H6" />
                </svg>
              </Link>
            ) : (
              <div className="flex flex-row items-center justify-center text-sm text-yellow-700 dark:text-yellow-500 bg-yellow-50 dark:bg-yellow-900/10 p-3 rounded-md border border-yellow-100 dark:border-yellow-800">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <span>Please connect to both accounts to access your playlists</span>
              </div>
            )}
          </div>
          
          <Link href="/en" className="inline-block mt-6 text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300">
            <span className="flex items-center">
